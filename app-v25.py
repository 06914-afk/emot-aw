        ...
        <body style="font-family: 'Microsoft JhengHei', sans-serif; color: #2E4436; background-color: #F6FAF4; padding: 20px;">
            <div style="max-width: 600px; margin: 0 auto; background-color: #FFFFFF; padding: 30px; border-radius: 12px; border: 1px solid #D5E5DE; box-shadow: 2px 4px 15px rgba(143, 168, 147, 0.1);">
                ...
                <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
                    ...
                    <tr>
                        <td style="padding: 10px; border: 1px solid #D5E5DE; font-weight: bold; color: #7A695A;">最常出現的身體反應</td>
                        <td style="padding: 10px; border: 1px solid #D5E5DE;">{row_data.get('最常出現的身體反應', '無')}</td>
                    </tr>
                    <tr>
                        <td style="padding: 10px; border: 1px solid #D5E5DE; font-weight: bold; color: #7A695A;">最常出現的情緒類別</td>
                        <td style="padding: 10px; border: 1px solid #D5E5DE;">{row_data.get('最常出現的情緒類別', '無')}</td>
                    </tr>
                    <tr>
                        <td style="padding: 10px; border: 1px solid #D5E5DE; font-weight: bold; color: #7A695A;">最常用的落地方式</td>
                        <td style="padding: 10px; border: 1px solid #D5E5DE;">{row_data.get('最常用的落地方式', '無')}</td>
                    </tr>
                    <tr>
                        <td style="padding: 10px; border: 1px solid #D5E5DE; font-weight: bold; color: #7A695A;">心得或對法師提問</td>
    ...
    if st.session_state.get("submitted_successfully", False):
        ...
        <div class="success-box">
            ...
            <ul>
                ...
                <li><b>最常出現的身體反應：</b>{st.session_state.get('last_submitted_body_reactions', '無')}</li>
                <li><b>最常出現的情緒類別：</b>{st.session_state.get('last_submitted_emotion_types', '無')}</li>
                <li><b>最常用的落地方式：</b>{st.session_state.get('last_submitted_grounding_methods', '無')}</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.write("---")
        st.markdown("#### 🌸 【新增統計題】請勾選您本週的情緒觀察概況：")
        
        body_reactions = st.multiselect(
            "【題目三】本週「最常出現的身體反應」： (可多選，或自行輸入新選項)",
            options=["肌肉緊繃", "胸悶/呼吸不順", "心跳加快", "腸胃不適", "頭暈/頭痛", "身體發熱/出汗", "疲憊無力", "無明顯反應"],
            key="student_body_reactions"
        )
        emotion_types = st.multiselect(
            "【題目四】本週「最常出現的情緒類別」： (可多選，或自行輸入新選項)",
            options=["貪 (執著/喜愛)", "嗔 (生氣/排斥/不耐煩)", "痴 (散亂/放空/不知所措)", "慢 (傲慢/比較)", "疑 (懷疑/不信任)", "焦慮/緊張", "平靜/喜悅"],
            key="student_emotion_types"
        )
        grounding_methods = st.multiselect(
            "【題目五】本週「最常用的落地方式」： (可多選，或自行輸入新選項)",
            options=["呼吸覺察/深呼吸", "身體掃描/放鬆", "正念步行/經行", "聽聞佛法/思惟理路", "觀功念恩/代人著想", "拜佛/持咒/念誦", "暫時轉移注意力"],
        ...
        if submit_btn:
            ...
            new_row = {
                ...
                "最常出現的身體反應": ", ".join(body_reactions) if body_reactions else "無",
                "最常出現的情緒類別": ", ".join(emotion_types) if emotion_types else "無",
                "最常用的落地方式": ", ".join(grounding_methods) if grounding_methods else "無"
            }
            ...
            if success:
                ...
                st.session_state["last_submitted_body_reactions"] = ", ".join(body_reactions) if body_reactions else "無"
                st.session_state["last_submitted_emotion_types"] = ", ".join(emotion_types) if emotion_types else "無"
                st.session_state["last_submitted_grounding_methods"] = ", ".join(grounding_methods) if grounding_methods else "無"
                st.rerun()
...
（若您剛剛已提交，系統同步可能有些許延遲，請忽略此訊息。再次感恩大家一同增長智慧與慈悲！🌱）"""
        ...
        if not leader_df.empty:
            ...
            display_cols = ["個人編號", "個人姓名", "當週填表張數", "累積填表張數"]
    ...
    if not is_authenticated:
        ...
        if not df.empty:
            ...
            st.table(stats_df[["組別", "保溫組長", "應填人數", "已填人數", "未填人數", "填報率"]].set_index("組別"))
        else:
        ...
        if not df.empty:
            ...
            st.table(filtered_df.set_index("時間戳記"))
