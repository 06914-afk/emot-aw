...
def normalize_group_string(raw_g):
    if not raw_g or str(raw_g).strip() in ["", "nan", "None"]:
        return raw_g
    s = str(raw_g).replace(' ', '').strip()
    m = re.match(r'^第(\d{1,2})組$', s)
    if m:
        return f"第 {m.group(1)} 組"
    return str(raw_g).strip()

def normalize_name_string(raw_n):
    if not raw_n or str(raw_n).strip() in ["", "nan", "None"]:
        return raw_n
    s = str(raw_n).strip()
    if ' ' in s:
        parts = s.split(' ')
...
def normalize_week_string(raw_str, official_weeks, ts_str=None):
    if not raw_str or str(raw_str).strip() in ["", "nan", "None"]:
        if ts_str:
            try:
                ts_dt = datetime.datetime.strptime(str(ts_str).strip()[:10], '%Y-%m-%d').date()
                for wk in official_weeks:
                    e_dt = get_week_end_date(wk)
                    if e_dt and ts_dt <= e_dt:
                        return wk
            except Exception:
                pass
        return raw_str
        
    s = str(raw_str).strip().replace('\xa0', ' ').replace('～', '~').replace('—', '~').replace('-', '~')
    match_code = re.search(r'(?:[Ww]|第)\s*(\d{1,2})\s*(?:週|周)?', s)
    if match_code:
    ...
    m_cn = re.search(r'第\s*([一二三四五六七八九十]{1,3})\s*[週周]', s)
    if m_cn:
        cn_str = m_cn.group(1)
        if cn_str in cn_num_map:
            w_num = cn_num_map[cn_str]
            std_code = f"W{w_num}"
            if std_code in week_code_map:
                return week_code_map[std_code]
    ...
    if os.path.exists(DB_FILE):
        try:
            ...
            if not df.empty and "時間戳記" in df.columns and "個人姓名" in df.columns and "當週回報期間" in df.columns:
                ...
                # 1. 規範化 組別 (如 '第10組' -> '第 10 組')
                if "組別" in df.columns:
                    norm_groups = [normalize_group_string(g) for g in df["組別"]]
                    if norm_groups != list(df["組別"]):
                        needs_resave = True
                    df["組別"] = norm_groups
                    
                # 2. 規範化 個人姓名 (如 '10-1 黃振育' -> '黃振育')
                norm_names = [normalize_name_string(n) for n in df["個人姓名"]]
                if norm_names != list(df["個人姓名"]):
                    needs_resave = True
                df["個人姓名"] = norm_names
                
                # 3. 規範化 當週回報期間 (如 'W1', '第1週', 'W1 ( 2026/03/29 ... )' -> 'W1 ( 2026/08/31 ~ 2026/09/05 )')
                norm_periods = []
                for idx, row in df.iterrows():
                    orig_p = row["當週回報期間"]
                    ts = row.get("時間戳記", "")
                    norm_p = normalize_week_string(orig_p, weeks_list, ts)
                    if norm_p != orig_p:
                        needs_resave = True
                ...
                # 排除重複填報：每個組員在同一週（當週回報期間）只能保留最後一筆（最新）資料
...
def save_record(new_row, gas_url=""):
    if "組別" in new_row:
        new_row["組別"] = normalize_group_string(new_row["組別"])
    if "個人姓名" in new_row:
        new_row["個人姓名"] = normalize_name_string(new_row["個人姓名"])
    if "當週回報期間" in new_row:
        ts = new_row.get("時間戳記", "")
        new_row["當週回報期間"] = normalize_week_string(new_row["當週回報期間"], weeks_list, ts)
