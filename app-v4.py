...
def generate_all_groups_docx_report(week, all_data, groups_data):
    ...
    run_h_sum = h_sum.add_run("📊 全營 18 保溫小組當週填報進度與張數總總覽")
    ...
    table_exec = doc.add_table(rows=1, cols=8)
    ...
    for g_name, g_info in groups_data.items():
        ...
        r_vals = [g_name, ldr, str(len(m_list_full)), str(sub_count), str(unsub_count), f"{g_sheets} 張", f"{avg_sheets:.1f} 張/人", f"{rate:.1f}%"]
    ...
    r_cmet = p_camp_met.add_run(f"💡 全營整體概況：全營 18 小組應填報共 {total_camp_m} 人，當週實際完成登記 {total_camp_sub} 人，未填報 {total_camp_unsub} 人，全營整體填報率為 {overall_rate:.1f}%。當週全營累積填寫情緒觀察表總計 {total_camp_sheets} 張，全營平均每人填寫 {overall_avg:.1f} 張。")
    ...
    run_h_group_sec = h_group_sec.add_run("💬 十八組各組繳交狀況表格與心得摘要（每組約兩百字）")
    ...
    for g_name, g_info in groups_data.items():
        ...
        table_g = doc.add_table(rows=1, cols=5)
        ...
        r_gm = p_gmet.add_run(f"💡 小組小結：應填 {len(roster_list)} 人，已填 {g_submitted_count} 人，未填 {len(roster_list) - g_submitted_count} 人。當週小組總填表張數：{g_total_sheets} 張。")
        ...
        p_sum_lbl.paragraph_format.space_before = Pt(4)
        p_sum_lbl.paragraph_format.space_after = Pt(2)
        r_s_lbl = p_sum_lbl.add_run(f"📝 {g_name} 本週觀照與心得摘要（每組約兩百字）：")
        r_s_lbl.bold = True
        r_s_lbl.font.size = Pt(10.5)
        r_s_lbl.font.color.rgb = RGBColor(27, 94, 32)
        
        summary_paragraph = build_group_summary_text(g_name, ldr, g_submissions, roster_list)
        p_sum_body = doc.add_paragraph()
        p_sum_body.paragraph_format.left_indent = Inches(0.2)
        p_sum_body.paragraph_format.space_after = Pt(14)
        r_s_body = p_sum_body.add_run(summary_paragraph)
        r_s_body.font.size = Pt(10)
        r_s_body.font.color.rgb = RGBColor(40, 40, 40)
...
{unsub_mentions}"""
    ...
    if not is_authenticated:
        ...
        if not df.empty:
            ...
            st.table(stats_df[["組別", "保溫組長", "應填人數", "已填人數", "未填人數", "當週填表張數", "平均每人填表數", "填報率"]].set_index("組別"))
            
            # 💡 v42 新增：總窗口一鍵導出全營 18 組每週情緒觀察總總結 Word 報告 (.docx)
            target_wk_label = target_report_week.split(' (')[0]
            st.write("---")
            st.markdown(f"### 📝 一鍵生成全營 18 組【{target_wk_label}】情緒觀察總總結報告 (.docx)")
            st.caption("自動產出包含全營 18 小組的繳交狀況表格、張數統計，以及各組心得之約兩百字精華摘要的完整 Word 報告。")
            ...
            btn_col_a1, btn_col_a2 = st.columns([1, 2])
            with btn_col_a1:
                st.download_button(
                    label="📥 一鍵導出全營 18 組總總結 Word 報告",
                    data=doc_all_bio,
            ...
            with btn_col_a2:
                st.success("✅ 全營 18 組總總結報告已生成！點擊左側按鈕即可下載 Word 檔。")
        else:
