...
def send_email_copy(receiver_email, row_data):
    ...
    try:
        ...
        subject = f"【情緒觀察營】填報確認副本 — {row_data['個人姓名']} 同修"
        ...
        <body style="font-family: 'Microsoft JhengHei', sans-serif; color: #2E4436; background-color: #F6FAF4; padding: 20px;">
            <div style="max-width: 600px; margin: 0 auto; background-color: #FFFFFF; padding: 30px; border-radius: 12px; border: 1px solid #D5E5DE; box-shadow: 2px 4px 15px rgba(143, 168, 147, 0.1);">
                ...
                <p>親愛的 <b>{row_data['個人姓名']}</b> 同修吉祥：</p>
                ...
                <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
                    ...
                    <tr>
                        <td style="padding: 10px; border: 1px solid #D5E5DE; font-weight: bold; color: #7A695A;">保溫組長</td>
                        <td style="padding: 10px; border: 1px solid #D5E5DE;">{row_data['保溫組長']}</td>
                    </tr>
                    <tr>
                ...
                <div style="background-color: #FCF5EC; border-left: 4px solid #E2A784; padding: 15px; border-radius: 6px; font-size: 0.95rem; color: #6D5144; margin-top: 20px;">
                    💡 <b>小提醒：</b>本信件為系統自動發送之確認副本。若您對申報數據有任何修正需求，請直接聯繫您的保溫組長 <b>{row_data['保溫組長']}</b>。再次感恩您一同增長慈悲與智慧！🌱
                </div>
                
                <p style="text-align: center; color: #7A695A; font-size: 0.85rem; margin-top: 35px; border-top: 1px solid #EAE0D5; padding-top: 15px;">
                    🌸 園區情緒觀察營 大組關懷系統 合十 🌸
                </p>
            </div>
        </body>
...
（若您剛剛已提交，系統同步可能有些許延遲，請忽略此訊息。再次感恩大家一同增長智慧與慈悲！🌱）"""
        ...
        if not leader_df.empty:
            ...
            leader_df_display = leader_df[["個人編號", "個人姓名", "當週填表張數", "累積填表張數", "心得或對法師提問", "時間戳記"]].set_index("個人編號")
