...
st.markdown('<div class="report-subtitle">提供全體保溫小組填表申報、組長溫馨關懷與總窗口全局統計 — 清新水彩版</div>', unsafe_allow_html=True)
...
if show_admin_login_screen:
    ...
    if st.session_state.get("submitted_successfully", False):
        ...
        cum_sheets = st.number_input("【題目二】您自 2026/8/17 起累計填寫了幾張情緒觀察表（含本週）？ *", min_value=0, max_value=1000, value=0, step=1, key="student_cum_sheets")
    ...
    if not is_authenticated:
        ...
        with col_l1:
            ...
            start_week = st.selectbox("開始週別：", weeks_list, index=0)
            end_week = st.selectbox("結束週別：", weeks_list, index=0)
