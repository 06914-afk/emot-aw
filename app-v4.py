...
def calibrate_reported_week(reported_week, ts_str, weeks):
    """
    💡 v51/v52 升級除錯檢查：精準判斷週次，對齊歷史舊版 W1/W2 數據，並自動校正未來誤填。
    """
    if not reported_week or not ts_str:
    ...
    curr_wk_idx = get_week_index_for_date(ts_dt, weeks)
    weekday = ts_dt.weekday() # 0=Mon..6=Sun
    
    rep_idx = -1
    for i, wk_str in enumerate(weeks):
        if wk_str == reported_week or wk_str.split(' (')[0] == reported_week.split(' (')[0]:
            rep_idx = i
            break
    ...
    # 規則：在週日～週三(6, 0, 1, 2)為補填上週期間，若誤選當週或未來週次(>=curr_wk_idx)，自動校正回上週(curr_wk_idx - 1)
    # 在週四～週六(3, 4, 5)，若誤選未來週次(>curr_wk_idx)，自動校正回當週(curr_wk_idx)
    if weekday in [6, 0, 1, 2]:
        if rep_idx >= curr_wk_idx and curr_wk_idx > 0:
            return weeks[curr_wk_idx - 1]
    else:
        if rep_idx > curr_wk_idx:
            return weeks[curr_wk_idx]
            
    return weeks[rep_idx]
...
def get_default_week_index(selectable_wks):
    try:
        ...
        # 週日(6)至週三(2)：維持在上週 (倒數第二個)
        if weekday in [6, 0, 1, 2]:
            if len(selectable_wks) >= 2:
                return len(selectable_wks) - 2
            return 0
        else:
            return len(selectable_wks) - 1
    except Exception:
        return len(selectable_wks) - 1
