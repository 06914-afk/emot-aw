...
def calibrate_reported_week(reported_week, ts_str, weeks):
    """
    💡 v50 新增除錯檢查：若填報人選擇了相對其提交時間戳記為未來的週次，自動校正回該時間點對應的有效週次。
    """
    ...
    reported_e_date = get_week_end_date(str(reported_week))
    if not reported_e_date:
        return reported_week
        
    # 若填報週次的結束日期嚴格大於提交當天，代表選錯為未來週次，執行校正
    if reported_e_date > ts_dt:
        return get_valid_week_for_date(ts_dt, weeks)
