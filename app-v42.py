...
def generate_all_groups_docx_report(week, all_data, groups_data):
    ...
    table_exec = doc.add_table(rows=1, cols=7)
    ...
    for g_name, g_info in groups_data.items():
        ...
        r_vals = [g_name, ldr, str(len(m_list_full)), str(sub_count), str(unsub_count), f"{g_sheets} 張", f"{rate:.1f}%"]
    ...
    r_cmet = p_camp_met.add_run(f"💡 全營整體概況：全營 18 小組應填報共 {total_camp_m} 人，當週實際完成登記 {total_camp_sub} 人，未填報 {total_camp_unsub} 人，全營整體填報率為 {overall_rate:.1f}%。當週全營累積填寫情緒觀察表總計 {total_camp_sheets} 張。")
